...
os.environ[
    "GOOGLE_APPLICATION_CREDENTIALS"] = "C:/Users/lixxc/PycharmProjects/cbnu_kioskAi/CBNU_Kiosk_main/realnew-399713-2378aee3660a.json"
...
def old_pay(request):
    ...
    if request.method == "POST":
        ...
        for menu_title, cnt in zip(menu_list, menu_counts):
            ...
            order_obj.menu = menu_obj
            order_obj.count = cnt
            order_obj.created = timezone.now()

            # Get the latest Customer and set it as the customer for this order
            latest_customer = Customer.objects.order_by('-id').first()
            order_obj.customer = latest_customer
